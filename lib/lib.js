...
export function covenantCard(cfg) {
  ...
  return [
    ...
    "- 完工门内容检查（assert 组 14-16，2026-08-28 主线同步）: 波次 diff 增行不得含凭据（secret scan；未加引号的 os.environ/process.env/config.x/self.x 引用值豁免，.md 仅 WARN）；删测试断言须 `- test-change: <path> — <reason>` 日志理由（test-change guard，含整文件删除）。",
